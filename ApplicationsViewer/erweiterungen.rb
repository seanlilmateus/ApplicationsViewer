...
module MateusErweiterung
  class ::SBElementArray
		...
		class ::NSAlert
				...
				def beginSheetModalForWindow window, completionHandler:completionHandler
						runner = NSAlertWithBlockHelper::AlertSheetCompletionHandlerRunner.new.initWithAlert self, 
																																																										completionHandler:completionHandler
						runner.startOnWindow window    
				end
				# NSAlertSecondButtonReturn
				# NSAlertFirstButtonReturn
				def self.beginAlertSheetWithTitle title, defaultButton:defaultButton, 
								alternateButton:alternateButton, otherButton:otherButton, alertStyle:alertStyle, 
								window:docWindow, completionHandler:completionHandler, 
								informativeTextWithFormat:msgFormat, showsSuppressionButton:supression
										NSUserDefaults.standardUserDefaults.setBool false, forKey: AlertSuppress
										...
										if defaults.boolForKey AlertSuppress
												...
												if (msgFormat)
														informationalText = msgFormat #, arguments:args
														alert.setInformativeText informationalText
												end
												alert.addButtonWithTitle defaultButton if defaultButton
												alert.addButtonWithTitle alternateButton if alternateButton
												alert.addButtonWithTitle otherButton if otherButton
												alert.setAlertStyle alertStyle if alertStyle
												# alert.suppressionButton.state = NSOnState
												alert.setShowsSuppressionButton supression if supression        
												alert.beginSheetModalForWindow docWindow, completionHandler:completionHandler
										end
				end
				def suppress
						NSUserDefaults.standardUserDefaults.setBool true, forKey: AlertSuppress
						NSUserDefaults.standardUserDefaults.synchronize
						"suppressed"
				end
		end
		
		# ^^ NSAlert With Blocks ends here
		class ::NSString
				def language
		...
		class MTextField < NSTextField
				...
				def drawRect dirtyRect
						...
						hightlightFrame = NSMakeRect(0.0, 10.0, self.bounds.size.width, self.bounds.size.height-10.0)
						...
						# black outline
						blackOutlineFrame = NSMakeRect(0.0, 0.0, self.bounds.size.width, self.bounds.size.height-1.0)
				
						if NSApp.isActive
								startColor = NSColor.colorWithCalibratedWhite 0.24, alpha:1.0
								endColor = NSColor.colorWithCalibratedWhite 0.374, alpha:1.0
								...
								endColor = NSColor.colorWithCalibratedWhite 0.558, alpha:1.0
								gradient = NSGradient.alloc.initWithStartingColor startColor, endingColor:endColor
						end
						#path = 
						gradient.drawInBezierPath NSBezierPath.bezierPathWithRoundedRect(blackOutlineFrame, xRadius:3.6, yRadius:3.6),angle:90.0
						# top inner shadow
						shadowFrame = NSMakeRect(1, 1, self.bounds.size.width - 2.0, 10.0)
						NSColor.colorWithCalibratedWhite(0.88, alpha:1.0).set
						NSBezierPath.bezierPathWithRoundedRect(shadowFrame, xRadius:2.9, yRadius:3.9).fill
				
						# main white area
						whiteFrame = NSMakeRect(1, 2, self.bounds.size.width-2.0, self.bounds.size.height-4.0)
		...
		class ::NSDate #DistanceOfTimeInWords  & Blocks
				...
				def enumerateByTimeInterval interval, endDate:endDate, usingBlock:block
						return if ((interval == 0.0 || !block) || (interval > 0.0 && self.compare(endDate) == NSOrderedDescending) ||
						...
						end while ((interval > 0.0 && currentDate.compare(endDate) != NSOrderedDescending) || 
